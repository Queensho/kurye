...
class KuryeApp extends StatelessWidget {
  ...
  @override Widget build(BuildContext context){
    Widget home;
    if(isCustomerPath){ home=AppDataService.instance.isSignedIn?const HomePixelPreview():const CustomerPhoneAuthPage(); }
    ...
    return MaterialApp(debugShowCheckedModeBanner:false,title:isCustomerPath?'Kurye Müşteri':'Kurye',theme:ThemeData(useMaterial3:true,scaffoldBackgroundColor:const Color(0xFFF7FBFF),visualDensity:VisualDensity.compact,colorScheme:ColorScheme.fromSeed(seedColor:const Color(0xFF168CF5))),builder:(context,child){if(child==null||isCustomerPath)return child??const SizedBox.shrink();final mq=MediaQuery.of(context);final scale=(mq.size.width/430).clamp(.84,1.0);return MediaQuery(data:mq.copyWith(textScaler:TextScaler.linear(scale)),child:Align(alignment:Alignment.topCenter,child:ConstrainedBox(constraints:const BoxConstraints(maxWidth:480),child:child)));},home:home);
  }
}
