  ...
  Widget build(BuildContext context) {
    ...
    if (info == null) {
      return Scaffold(
        ...
        body: Center(child: Padding(padding: const EdgeInsets.all(24), child: Text(error ?? 'Kurye bilgisi alınamadı.'))),
      );
    }
    ...
    if (pickupLat != null && pickupLng != null) {
      markers.add(Marker(point: LatLng(pickupLat, pickupLng), width: 52, height: 52, child: _pin(blue, Icons.trip_origin_rounded)));
    }
    ...
    if (dropoffLat != null && dropoffLng != null) {
      markers.add(Marker(point: LatLng(dropoffLat, dropoffLng), width: 52, height: 52, child: _pin(green, Icons.location_on_rounded)));
    }
            ...
            Padding(
              ...
              child: Row(children: [
                ...
                const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('Kurye bulundu!', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w900, color: navy)),
                  Text('Kurye alım noktasına geliyor.', style: TextStyle(color: blue, fontWeight: FontWeight.w800)),
                ])),
              ]),
            ),
            Expanded(
              child: FlutterMap(
                options: MapOptions(initialCenter: center, initialZoom: 13.5, minZoom: 5, maxZoom: 18),
                children: [
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Text('$vehicle • ${d['shipment_status'] ?? 'accepted'}', style: const TextStyle(color: muted, fontWeight: FontWeight.w600)),
                  ])),
                  if (phone != null && phone.isNotEmpty) IconButton.filled(onPressed: () {}, icon: const Icon(Icons.phone_rounded)),
                ]),
                ...
                const Text('Konum, kurye hareket ettikçe gerçek zamanlı güncellenir.', style: TextStyle(color: muted, fontSize: 12)),
              ]),
            ),
          ],
        ),
