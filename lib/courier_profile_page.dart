...
class _CourierProfilePageState extends State<CourierProfilePage> {
  ...
  int get _completed =>
      shipments.where((e) => e['status'] == 'delivered').length;
  int get _countable =>
      shipments.where((e) => e['status'] != 'cancelled').length;
  int get _completionRate => _countable == 0
      ? 0
      : ((_completed / _countable) * 100).round().clamp(0, 100);
  double get _rating {
  ...
  double get _todayDistance {
    ...
    for (final item in shipments) {
      final d = DateTime.tryParse((item['created_at'] ?? '').toString())
          ?.toLocal();
      if (d != null &&
          d.year == now.year &&
          d.month == now.month &&
  ...
  int get _todayDeliveries {
    ...
    return shipments.where((item) {
      final d = DateTime.tryParse(
        (item['delivered_at'] ?? item['created_at'] ?? '').toString(),
      )?.toLocal();
  ...
  Future<void> _openEdit() async {
    await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) =>
            CourierProfileEditPage(profile: profile, courier: courier),
      ),
    );
    await _load();
  }
        ...
        Container(
          decoration: const BoxDecoration(
            gradient: LinearGradient(
              ...
              colors: [deepPurple, Color(0xFF5B26CE)],
            ),
    ...
    child: Container(
      ...
      child: Row(
        children: [
          Stack(
            ...
            children: [
              CircleAvatar(
                ...
                backgroundImage: _avatar.isEmpty ? null : NetworkImage(_avatar),
                child: _avatar.isEmpty
                    ? Icon(Icons.person_rounded, size: 42 * s, color: purple)
                    : null,
              ),
              Positioned(
                right: -1,
                ...
                child: Container(
                  ...
                  child: Icon(Icons.check, size: 10 * s, color: Colors.white),
                ),
              ),
            ],
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Uber Kurye • $_city',
                  style: TextStyle(color: muted, fontSize: 10.5 * s),
                ),
              ],
            ),
          ),
          OutlinedButton.icon(
            onPressed: _openEdit,
            icon: Icon(Icons.edit_outlined, size: 16 * s),
            label: Text(
              'Profili Düzenle',
              style: TextStyle(fontSize: 9.5 * s, fontWeight: FontWeight.w800),
            ),
  ...
  Widget _statCard(
    ...
    child: Column(
      ...
      children: [
        Row(
          children: [
            Icon(icon, color: color, size: 19 * s),
            SizedBox(width: 5 * s),
            Flexible(
              child: Text(
                value,
  ...
  Widget _trust(double s) => Padding(
    padding: EdgeInsets.symmetric(horizontal: 16 * s),
    child: Container(
      height: 58 * s,
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Performansını yüksek tut, avantajları artır.',
                  style: TextStyle(color: muted, fontSize: 9 * s),
                ),
              ],
            ),
          ),
          Icon(Icons.chevron_right_rounded, color: navy, size: 23 * s),
        ],
      ),
    ),
  );
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              Text(
                'Hafta ${_money(week)} • Ay ${_money(month)}',
                style: TextStyle(color: muted, fontSize: 9 * s),
              ),
            ],
          ),
  ...
  Widget _dayStats(double s) => Padding(
    padding: EdgeInsets.symmetric(horizontal: 16 * s),
  ...
  Widget _bottomNav(double s) =>
      CourierBottomNav(currentIndex: 3, onTap: _navigateBottom);
}

class CourierProfileEditPage extends StatefulWidget {
  const CourierProfileEditPage({
    super.key,
...
class _CourierProfileEditPageState extends State<CourierProfileEditPage> {
  ...
  void initState() {
    ...
    vehicle = TextEditingController(
      text:
          (widget.courier['vehicle_model'] ??
                  widget.courier['vehicle_type'] ??
                  '')
              .toString(),
    );
    plate = TextEditingController(
      text: (widget.courier['plate'] ?? '').toString(),
    );
    color = TextEditingController(
      text: (widget.courier['vehicle_color'] ?? '').toString(),
    );
    license = TextEditingController(
      text: (widget.courier['license_class'] ?? '').toString(),
    );
  }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      try {
        await data.client
            .from('profiles')
            .update({'city': city.text.trim()})
            .eq('id', data.userId);
      } catch (_) {}
      final update = <String, dynamic>{};
      if (widget.courier.containsKey('vehicle_model'))
        update['vehicle_model'] = vehicle.text.trim();
      if (widget.courier.containsKey('plate'))
        update['plate'] = plate.text.trim().toUpperCase();
      if (widget.courier.containsKey('vehicle_color'))
        update['vehicle_color'] = color.text.trim();
      if (widget.courier.containsKey('license_class'))
        update['license_class'] = license.text.trim().toUpperCase();
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        _sectionTitle('Temel Bilgiler', s),
                        ...
                        _group([
                          ...
                          _field(Icons.location_on_outlined, 'Şehir', city, s),
                        ], s),
                        SizedBox(height: 20 * s),
                        _sectionTitle('Araç Bilgileri', s),
                        SizedBox(height: 10 * s),
                        _group([
                          _field(Icons.two_wheeler_rounded, 'Araç', vehicle, s),
                          _field(Icons.pin_outlined, 'Plaka', plate, s),
                          _field(Icons.water_drop_outlined, 'Renk', color, s),
                          _field(
                        ...
                        SizedBox(height: 20 * s),
                        _sectionTitle('Hesap ve Güvenlik', s),
                        SizedBox(height: 10 * s),
                        _menuGroup(s),
                        ...
                        SizedBox(
                          ...
                          child: FilledButton.icon(
                            onPressed: _logout,
                            icon: Icon(Icons.logout_rounded, size: 19 * s),
                            label: Text(
                              'Hesaptan Çıkış Yap',
                              style: TextStyle(
                                fontSize: 11 * s,
  ...
  Widget _editHero(String avatarUrl, double s) => Container(
    height: 250 * s,
    decoration: const BoxDecoration(
      gradient: LinearGradient(
        begin: Alignment.topLeft,
        ...
        Align(
          ...
          child: Column(
            ...
            children: [
                    ...
                    CircleAvatar(
                      ...
                      backgroundImage: avatarBytes != null
                          ...
                          : (avatarUrl.isNotEmpty
                                ...
                                : null),
                      child: avatarBytes == null && avatarUrl.isEmpty
                          ? Icon(
              ...
              SizedBox(height: 10 * s),
              Text(
                name.text.isEmpty ? 'Kurye' : name.text,
                style: TextStyle(
                  color: Colors.white,
                  fontSize: 16 * s,
                  fontWeight: FontWeight.w900,
              ...
              SizedBox(height: 3 * s),
              Text(
                'Uber Kurye • ${city.text.isEmpty ? 'İstanbul' : city.text}',
                style: TextStyle(
  ...
  Widget _sectionTitle(String text, double s) => Text(
    text,
    style: TextStyle(
      color: navy,
      fontSize: 15 * s,
  ...
  Widget _group(List<Widget> rows, double s) => Container(
    decoration: BoxDecoration(
      color: Colors.white,
    ...
    child: Column(
      children: [
        for (int i = 0; i < rows.length; i++) ...[
          rows[i],
          if (i != rows.length - 1)
            Divider(height: 1, indent: 48 * s, color: const Color(0xFFEDEBF2)),
        ],
      ],
    ),
  );
  Widget _field(
    ...
    child: Row(
      children: [
        SizedBox(
          width: 48 * s,
          child: Icon(icon, color: const Color(0xFF6F7186), size: 21 * s),
        ),
        SizedBox(
          width: 90 * s,
          child: Text(
        ...
        Padding(
          ...
          child: Icon(Icons.chevron_right_rounded, color: muted, size: 20 * s),
        ),
      ],
    ),
  );
  Widget _menuGroup(double s) => Container(
    decoration: BoxDecoration(
      color: Colors.white,
      borderRadius: BorderRadius.circular(18 * s),
      border: Border.all(color: const Color(0xFFE7E5ED)),
    ...
    child: Column(
      children: [
        _menuRow(Icons.shield_outlined, 'Şifreyi Değiştir', null, s),
        _divider(s),
        _menuRow(
        ...
        _divider(s),
        _menuRow(Icons.visibility_outlined, 'Gizlilik Ayarları', null, s),
      ],
    ),
  );
  Widget _menuRow(
    ...
    child: SizedBox(
      ...
      child: Row(
        children: [
          ...
          SizedBox(width: 6 * s),
          Icon(Icons.chevron_right_rounded, color: muted, size: 20 * s),
          SizedBox(width: 10 * s),
        ],
      ),
    ),
