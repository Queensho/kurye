...
class HomePixelPreview extends StatelessWidget {
    ...
    return Scaffold(
      ...
      body: LayoutBuilder(builder: (context, c) {
        ...
        return Column(children: [
          ...
          Transform.translate(offset: Offset(0, -2 * y), child: Padding(padding: EdgeInsets.symmetric(horizontal: 10 * sx), child: PromoBannerWidget(height: 96 * y, borderRadius: BorderRadius.circular(18 * sx)))),
          SizedBox(height: 7 * y),
              ...
              child: Row(children: [
                ...
                FutureBuilder<List<Map<String, dynamic>>>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (rows.isNotEmpty) {
                      final defaults = rows.where((e) => e['is_default'] == true).toList();
                      selected = defaults.isNotEmpty ? defaults.first : rows.first;
                    }
                    final title = selected == null ? 'Konum seçilmedi' : (selected['label'] ?? 'Kayıtlı adres').toString();
                    final address = selected == null ? 'Profilimden adres ekle' : (selected['address_line'] ?? '').toString();
                    return SizedBox(
                      width: 205 * sx,
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Text(title, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 17 * fs, fontWeight: FontWeight.w800, color: deepBlue)),
  ...
  Widget _recent(BuildContext context, double sx, double y, double fs) => Column(children: [
        Row(children: [
          ...
          InkWell(onTap: () => _openMyShipments(context), child: Padding(padding: EdgeInsets.symmetric(horizontal: 5 * sx, vertical: 4 * y), child: Text('Tümünü Gör', style: TextStyle(fontSize: 10 * fs, color: blue, fontWeight: FontWeight.w700)))),
        ]),
        SizedBox(height: 6 * y),
        StreamBuilder<List<Map<String, dynamic>>>(
          ...
          builder: (context, snapshot) {
            ...
            if (snapshot.connectionState == ConnectionState.waiting && rows.isEmpty) {
              return Container(height: 66 * y, alignment: Alignment.center, decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18 * sx)), child: const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2)));
            }
            if (rows.isEmpty) {
              return InkWell(
                onTap: () => _openCreateShipment(context),
                child: Container(
                  ...
                  child: Row(children: [Icon(Icons.inventory_2_outlined, color: blue, size: 27 * fs), SizedBox(width: 10 * sx), Expanded(child: Text('Henüz gönderin yok', style: TextStyle(fontSize: 12 * fs, color: muted, fontWeight: FontWeight.w700))), const Icon(Icons.add_circle_outline_rounded, color: blue)]),
                ),
              );
            }
            final item = rows.first;
            final status = (item['status'] ?? '').toString();
            final labels = <String, String>{'searching': 'Kurye aranıyor', 'accepted': 'Kurye geliyor', 'at_pickup': 'Kurye alımda', 'picked_up': 'Gönderin yolda', 'at_dropoff': 'Teslimat noktasında', 'delivered': 'Teslim edildi', 'cancelled': 'İptal edildi'};
            ...
            final date = dt == null ? '' : '${dt.day.toString().padLeft(2, '0')}.${dt.month.toString().padLeft(2, '0')} ${dt.hour.toString().padLeft(2, '0')}:${dt.minute.toString().padLeft(2, '0')}';
            final done = status == 'delivered';
            final cancelled = status == 'cancelled';
            final badgeColor = done ? const Color(0xFF11843A) : cancelled ? const Color(0xFFD94A4A) : blue;
            final badgeBg = done ? const Color(0xFFDDF8E4) : cancelled ? const Color(0xFFFFE7E7) : const Color(0xFFEAF4FF);
            return InkWell(
              ...
              child: Container(
                ...
                child: Row(children: [
                  ...
                  SizedBox(width: 8 * sx),
                  Expanded(child: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [Text(code, style: TextStyle(fontSize: 10 * fs, fontWeight: FontWeight.w700)), Text('$pickup → $dropoff', maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 12 * fs, fontWeight: FontWeight.w800)), Text(labels[status] ?? 'Gönderi güncelleniyor', style: TextStyle(fontSize: 9 * fs, color: muted))])),
                  Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.end, children: [Text(date, style: TextStyle(fontSize: 8 * fs, color: muted)), SizedBox(height: 6 * y), Container(padding: EdgeInsets.symmetric(horizontal: 7 * sx, vertical: 4 * y), decoration: BoxDecoration(color: badgeBg, borderRadius: BorderRadius.circular(12 * sx)), child: Text(labels[status] ?? status, style: TextStyle(fontSize: 8 * fs, color: badgeColor, fontWeight: FontWeight.w700)))]),
                ]),
              ),
            );
          },
