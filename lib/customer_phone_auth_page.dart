...
class _CustomerPhoneAuthPageState extends State<CustomerPhoneAuthPage>{
  ...
  bool validEmail()=>RegExp(r'^[^\s@]+@[^\s@]+\.[^\s@]+$').hasMatch(email.text.trim());

  Future<void> submit() async {
    if(!validEmail()){setState(()=>error='Geçerli bir e-posta adresi gir.');return;}
    ...
    try{
      if(isRegister){
        await AppDataService.instance.signUpWithEmailPassword(email:email.text.trim(),password:password.text);
      }else{
        await AppDataService.instance.signInWithEmailPassword(email:email.text.trim(),password:password.text);
      }
      if(mounted) Navigator.of(context).pushReplacement(MaterialPageRoute(builder:(_)=>const HomePixelPreview()));
    }catch(e){
      if(!mounted)return;
      final text=e.toString();
      setState(()=>error=text.contains('Invalid login credentials')?'E-posta veya şifre hatalı.':text.replaceFirst('StateError: ',''));
    }finally{if(mounted)setState(()=>loading=false);}
  }

  InputDecoration fieldDecoration(String label,{Widget? suffix})=>InputDecoration(
    labelText:label,filled:true,fillColor:Colors.white,suffixIcon:suffix,
  ...
  @override Widget build(BuildContext context)=>Scaffold(
    ...
    body:SafeArea(child:Center(child:SingleChildScrollView(padding:const EdgeInsets.all(28),child:ConstrainedBox(
      ...
      child:Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[
        ...
        Text(isRegister?'E-posta adresin ve şifrenle müşteri hesabını oluştur.':'E-posta adresin ve şifrenle giriş yap.',textAlign:TextAlign.center,style:const TextStyle(color:Color(0xff718096),fontSize:14)),
        const SizedBox(height:26),
        Container(padding:const EdgeInsets.all(4),decoration:BoxDecoration(color:const Color(0xffeaf3fb),borderRadius:BorderRadius.circular(18)),child:Row(children:[
        ...
        TextField(controller:email,keyboardType:TextInputType.emailAddress,autocorrect:false,decoration:fieldDecoration('E-posta').copyWith(hintText:'ornek@mail.com',prefixIcon:const Icon(Icons.mail_outline_rounded))),
        ...
        FilledButton(onPressed:loading?null:submit,style:FilledButton.styleFrom(backgroundColor:const Color(0xff168cf5),minimumSize:const Size.fromHeight(58),shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(20))),child:loading?const SizedBox(width:22,height:22,child:CircularProgressIndicator(strokeWidth:2,color:Colors.white)):Text(isRegister?'Kayıt Ol':'Giriş Yap',style:const TextStyle(fontSize:17,fontWeight:FontWeight.w900))),
        const SizedBox(height:18),
  ...
  Widget _tab(String text,bool selected,VoidCallback onTap)=>InkWell(onTap:onTap,borderRadius:BorderRadius.circular(15),child:Container(
