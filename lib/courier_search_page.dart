...
class CourierSearchPage extends StatelessWidget {
  ...
  static const _center = LatLng(41.0678, 28.9951);
          ...
          return SingleChildScrollView(
            ...
            child: Column(children: [
              Padding(
                ...
                child: Column(children: [
                  Row(children: [_round(s, Icons.arrow_back_rounded, () => Navigator.of(context).pop()), const Spacer()]),
                  SizedBox(height: 4 * s),
                  SizedBox(
                    height: 178 * s,
                    child: Stack(children: [
                      Positioned(
                        left: 0, top: 12 * s, right: 125 * s,
                      ...
                      Positioned(right: -4 * s, bottom: 8 * s, width: 145 * s, height: 160 * s, child: Image.asset('assets/images/kurye_header_hd.png', fit: BoxFit.contain, alignment: Alignment.bottomRight)),
                    ]),
                  ),
                  SizedBox(height: 8 * s),
                  Container(
  ...
  Widget _map(double s) {
    const couriers = [LatLng(41.0744, 28.9885), LatLng(41.0710, 29.0047), LatLng(41.0607, 29.0074)];
    return SizedBox(
      height: 330 * s, width: double.infinity,
      child: Stack(children: [
        FlutterMap(
          options: const MapOptions(initialCenter: _center, initialZoom: 13.8, minZoom: 10, maxZoom: 18),
          children: [
            TileLayer(urlTemplate: 'https://tile.openstreetmap.org/{z}/{x}/{y}.png', userAgentPackageName: 'com.queensho.kurye'),
            MarkerLayer(markers: [
              Marker(point: _center, width: 84 * s, height: 84 * s, child: Container(decoration: BoxDecoration(shape: BoxShape.circle, color: blue.withValues(alpha: .18)), alignment: Alignment.center, child: Container(width: 48 * s, height: 48 * s, decoration: const BoxDecoration(shape: BoxShape.circle, color: blue), child: Icon(Icons.location_on_rounded, color: Colors.white, size: 28 * s)))),
              ...couriers.map((p) => Marker(point: p, width: 62 * s, height: 62 * s, child: _courier(s))),
            ]),
          ],
        ),
        Positioned(left: 16 * s, top: 14 * s, child: Container(padding: EdgeInsets.symmetric(horizontal: 13 * s, vertical: 9 * s), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(15 * s), boxShadow: const [BoxShadow(color: Color(0x16000000), blurRadius: 12)]), child: Row(children: [Icon(Icons.radar_rounded, color: blue, size: 20 * s), SizedBox(width: 7 * s), Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Kuryeler bölgede', style: TextStyle(fontSize: 10.5 * s, fontWeight: FontWeight.w800, color: navy)), Text('3 kurye yakınında', style: TextStyle(fontSize: 9.5 * s, color: muted))])]))),
  ...
  Widget _courier(double s) => Container(width: 58 * s, height: 58 * s, padding: EdgeInsets.all(5 * s), decoration: BoxDecoration(color: Colors.white, shape: BoxShape.circle, border: Border.all(color: const Color(0xFFB9DAFF), width: 2), boxShadow: const [BoxShadow(color: Color(0x18000000), blurRadius: 8)]), child: Image.asset('assets/images/motosiklet_hd.png', fit: BoxFit.contain));
  ...
  Widget _details(double s) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      SizedBox(height: 14 * s), _addressRow(s, blue, 'Alım Adresi', 'Şişli, Mecidiyeköy', '2.3 km'), SizedBox(height: 12 * s), _addressRow(s, const Color(0xFF12CC8A), 'Teslimat Adresi', 'Kadıköy, İstanbul', '12.8 km'),
    ]),
  );
