...
class CourierAssignedJobsPage extends StatelessWidget {
  ...
  String _status(String raw) => switch (raw) {
    'accepted' => 'Alım bekleniyor',
    'at_pickup' => 'Alım noktasında',
    'picked_up' => 'Teslimatta',
        ...
        child: LayoutBuilder(builder: (context, constraints) {
          ...
          return Column(children: [
            Container(
              ...
              color: Colors.white,
              child: Row(children: [
                Expanded(child: Text('Atananlar',style: TextStyle(color: navy,fontSize: 23*s,fontWeight: FontWeight.w900))),
                StreamBuilder<List<Map<String,dynamic>>>(
                  stream: data.client.from('shipments').stream(primaryKey:['id']).eq('courier_id',data.userId),
                  builder: (_, snap) {
                    final count = (snap.data ?? const <Map<String,dynamic>>[]).where((e)=>!['delivered','cancelled'].contains((e['status']??'').toString())).length;
                    return Container(padding: EdgeInsets.symmetric(horizontal:10*s,vertical:6*s),decoration:BoxDecoration(color:const Color(0xFFFFEEE7),borderRadius:BorderRadius.circular(14*s)),child:Text('$count aktif',style:TextStyle(color:orange,fontSize:9*s,fontWeight:FontWeight.w800)));
                  },
                ),
              ]),
            ),
            Expanded(child: StreamBuilder<List<Map<String,dynamic>>>(
              stream: data.client.from('shipments').stream(primaryKey:['id']).eq('courier_id',data.userId).order('claimed_at',ascending:false).map((rows)=>List<Map<String,dynamic>>.from(rows.where((e)=>!['delivered','cancelled'].contains((e['status']??'').toString())))),
              builder: (context,snapshot) {
                if(snapshot.connectionState==ConnectionState.waiting&&!snapshot.hasData)return const Center(child:CircularProgressIndicator(color:orange));
                final rows=snapshot.data??const <Map<String,dynamic>>[];
                if(rows.isEmpty)return Center(child:Padding(padding:EdgeInsets.all(28*s),child:Column(mainAxisSize:MainAxisSize.min,children:[Icon(Icons.work_outline_rounded,color:muted,size:42*s),SizedBox(height:10*s),Text('Atanmış aktif işin yok',style:TextStyle(color:navy,fontSize:14*s,fontWeight:FontWeight.w900)),SizedBox(height:4*s),Text('Havuzdan birden fazla iş alabilirsin.',textAlign:TextAlign.center,style:TextStyle(color:muted,fontSize:10*s))])));
                      ...
                      child:Container(
                        ...
                        child:Row(children:[
                          ...
                          Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
                            Row(children:[Expanded(child:Text(code.startsWith('#')?code:'#$code',style:TextStyle(color:navy,fontSize:12*s,fontWeight:FontWeight.w900))),if(distance!=null)Text('$distance km',style:TextStyle(color:purple,fontSize:9*s,fontWeight:FontWeight.w800))]),
                            SizedBox(height:3*s),
                            Text(pickup,maxLines:1,overflow:TextOverflow.ellipsis,style:TextStyle(color:muted,fontSize:9.5*s)),
                            Row(children:[Icon(Icons.arrow_downward_rounded,color:orange,size:12*s),SizedBox(width:3*s),Expanded(child:Text(dropoff,maxLines:1,overflow:TextOverflow.ellipsis,style:TextStyle(color:muted,fontSize:9.5*s)))]),
                          ])),
                          SizedBox(width:8*s),
                          Column(crossAxisAlignment:CrossAxisAlignment.end,children:[
                            ...
                            SizedBox(height:4*s),
                            Text(_status(status),style:TextStyle(color:orange,fontSize:8.5*s,fontWeight:FontWeight.w700)),
                            SizedBox(height:4*s),Icon(Icons.chevron_right_rounded,color:muted,size:18*s),
                          ]),
                        ]),
                      ),
                    );
