...
class AppDataService {
  ...
  Future<void> signUpWithEmailPassword({required String email, required String password}) async {
    final res = await client.auth.signUp(email: email, password: password, data: const {'role': 'customer'});
    if (res.session == null) {
      throw StateError('Hesabın oluşturuldu. E-posta adresine gelen doğrulama bağlantısını onayladıktan sonra giriş yap.');
    }
    await ensureProfile();
    await client.from('profiles').update({'email': email, 'updated_at': DateTime.now().toUtc().toIso8601String()}).eq('id', userId);
    await ensureAccountActive();
  }
  ...
  Future<void> startCourierLocationTracking() async {
    ...
    _courierPositionSubscription = Geolocator.getPositionStream(locationSettings: settings).listen(
      (position) async { try { await updateCourierLocation(position.latitude, position.longitude); } catch (_) {} },
      onError: (_) async { await stopCourierLocationTracking(); },
    );
  }
  ...
  Future<Map<String,dynamic>> getCourierEarningsSummary() async {
    try { final value = await client.rpc('get_courier_earnings_summary'); return Map<String,dynamic>.from(value as Map); }
    on PostgrestException catch (e) { if(e.message.contains('account_suspended')) throw StateError('Bu hesap askıya alınmış.'); rethrow; }
  }

  Future<Map<String,dynamic>?> getCourierBankAccount() async {
    try { final value = await client.rpc('get_courier_bank_account'); if (value == null) return null; return Map<String,dynamic>.from(value as Map); }
    on PostgrestException catch (e) { if(e.message.contains('account_suspended')) throw StateError('Bu hesap askıya alınmış.'); rethrow; }
  }
  ...
  Future<Map<String,dynamic>> saveCourierBankAccount({required String accountHolder,required String bankName,required String iban}) async {
    try {
      final value = await client.rpc('save_courier_bank_account',params:{'p_account_holder':accountHolder,'p_bank_name':bankName,'p_iban':iban});
      return Map<String,dynamic>.from(value as Map);
    } on PostgrestException catch(e) {
  ...
  Future<Map<String,dynamic>> requestCourierPayout(int amount) async {
    try { final value = await client.rpc('request_courier_payout',params:{'p_amount':amount}); return Map<String,dynamic>.from(value as Map); }
    on PostgrestException catch(e) {
  ...
  Stream<List<Map<String,dynamic>>> watchCourierEarnings() => client.from('courier_earnings').stream(primaryKey:['id']).eq('courier_id', userId).order('created_at', ascending:false).map((rows)=>List<Map<String,dynamic>>.from(rows));
  Stream<List<Map<String,dynamic>>> watchCourierPayouts() => client.from('courier_payouts').stream(primaryKey:['id']).eq('courier_id', userId).order('created_at', ascending:false).map((rows)=>List<Map<String,dynamic>>.from(rows));
