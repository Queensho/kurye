...
class _CreateShipmentPageState extends State<CreateShipmentPage> {
              ...
              child: FlutterMap(
                ...
                children: [
                  ...
                  MarkerLayer(markers: [
                    ...
                    Marker(point: b.point, width: 54, height: 54, child: _mapPin(const Color(0xFF19C983), Icons.location_on_rounded)),
                  ]),
                ],
              ),
            ),
    ...
    showModalBottomSheet(
      ...
      builder: (_) => const Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Nasıl çalışır?', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: navy)),
          SizedBox(height: 12),
          Text('Adresleri seç, gönderi detaylarını gir ve gönderiyi oluştur. Sistem uygun kuryeyi aramaya başlar.'),
        ]),
      ),
    );
  }

  void _showVehicleHelp() {
    showModalBottomSheet(
      context: context,
      backgroundColor: Colors.white,
      shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(24))),
      builder: (_) => const Padding(
        padding: EdgeInsets.fromLTRB(20, 18, 20, 28),
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Taşıma türü seçimi', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: navy)),
          SizedBox(height: 12),
          Text('Küçük ve hızlı gönderiler için Motosiklet, daha büyük veya hacimli gönderiler için Araç seçebilirsin.'),
        ]),
      ),
    );
  }
  ...
  void _showIntermediateStop() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text('Ara durak özelliği aktif. Bir sonraki adımda adres seçimi bu noktaya bağlanabilir.'),
        ...
        child: Column(
          ...
          children: [
            Padding(
              ...
              child: Row(children: [Expanded(child: Text(title, style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w900, color: navy))), IconButton(onPressed: () => Navigator.pop(context), icon: const Icon(Icons.close_rounded))]),
            ),
  ...
  Widget _mapPin(Color color, IconData icon) => Container(
        decoration: BoxDecoration(color: Colors.white, shape: BoxShape.circle, border: Border.all(color: color, width: 2), boxShadow: const [BoxShadow(color: Color(0x22000000), blurRadius: 12)]),
  ...
  Widget _mapStat(IconData icon, String value, String label) => Container(
        ...
        child: Row(children: [Icon(icon, color: blue), const SizedBox(width: 9), Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(value, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w900, color: navy)), Text(label, style: const TextStyle(fontSize: 10, color: muted))])]),
      );
          ...
          return SingleChildScrollView(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                SizedBox(width: 14 * s),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Gönderi Oluştur', style: TextStyle(fontSize: 25 * s, fontWeight: FontWeight.w900, color: navy)), Text('Hızlı, güvenli ve kolay', style: TextStyle(fontSize: 14 * s, color: muted))])),
              ...
              Row(children: [Expanded(child: Text('Taşıma Türü', style: TextStyle(fontSize: 19 * s, fontWeight: FontWeight.w900, color: navy))), InkWell(onTap: _showVehicleHelp, child: Row(children: [Text('Hangisini seçmeliyim?', style: TextStyle(fontSize: 11 * s, color: blue)), Icon(Icons.chevron_right_rounded, color: blue, size: 18 * s)]))]),
              SizedBox(height: 10 * s),
              Row(children: [Expanded(child: _vehicle(s, 0, 'Motosiklet', 'Hızlı ve pratik', 'Genellikle 10–30 dk', 'assets/images/motosiklet_hd.png')), SizedBox(width: 10 * s), Expanded(child: _vehicle(s, 1, 'Araç', 'Daha büyük gönderiler', 'Genellikle 20–60 dk', 'assets/images/arac_hd.png'))]),
              SizedBox(height: 16 * s),
              _card(s, Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [Expanded(child: Text('Adres Bilgileri', style: TextStyle(fontSize: 18 * s, fontWeight: FontWeight.w900, color: navy))), InkWell(onTap: _showMapPreview, borderRadius: BorderRadius.circular(12 * s), child: Padding(padding: EdgeInsets.symmetric(horizontal: 6 * s, vertical: 5 * s), child: Row(children: [Icon(Icons.map_outlined, color: blue, size: 19 * s), SizedBox(width: 5 * s), Text(hasAddresses ? 'Haritada Gör' : 'Haritadan Seç', style: TextStyle(fontSize: 11.5 * s, color: blue, fontWeight: FontWeight.w800))])))]),
                ...
                if (hasAddresses) ...[SizedBox(height: 10 * s), InkWell(onTap: _showMapPreview, borderRadius: BorderRadius.circular(14 * s), child: Container(height: 43 * s, padding: EdgeInsets.symmetric(horizontal: 12 * s), decoration: BoxDecoration(color: const Color(0xFFEAF4FF), borderRadius: BorderRadius.circular(14 * s)), child: Row(children: [Icon(Icons.map_rounded, color: blue, size: 21 * s), SizedBox(width: 9 * s), Expanded(child: Text('Rotayı haritada gör', style: TextStyle(fontSize: 11.5 * s, color: blue, fontWeight: FontWeight.w800))), Icon(Icons.chevron_right_rounded, color: blue, size: 20 * s)])))],
                SizedBox(height: 10 * s),
                InkWell(onTap: _showIntermediateStop, borderRadius: BorderRadius.circular(14 * s), child: Container(height: 43 * s, padding: EdgeInsets.symmetric(horizontal: 12 * s), decoration: BoxDecoration(color: const Color(0xFFEAF4FF), borderRadius: BorderRadius.circular(14 * s), border: Border.all(color: const Color(0xFFB9DAFF))), child: Row(children: [Container(width: 26 * s, height: 26 * s, decoration: const BoxDecoration(color: Color(0xFF6EB5FF), shape: BoxShape.circle), child: Icon(Icons.add_rounded, color: Colors.white, size: 19 * s)), SizedBox(width: 10 * s), Text('Ara durak ekle (isteğe bağlı)', style: TextStyle(fontSize: 11.5 * s, color: blue, fontWeight: FontWeight.w700))]))),
              ])),
              AnimatedSwitcher(duration: const Duration(milliseconds: 250), child: hasAddresses ? Padding(key: const ValueKey('price'), padding: EdgeInsets.only(top: 16 * s), child: calculatingRoute ? _calculatingCard(s) : _priceCard(s)) : const SizedBox.shrink(key: ValueKey('empty'))),
              SizedBox(height: 16 * s),
              _card(s, Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Container(
                  ...
                  child: Row(children: [Icon(Icons.chat_bubble_outline_rounded, color: const Color(0xFF65748A), size: 20 * s), SizedBox(width: 10 * s), Expanded(child: TextField(controller: noteController, maxLines: 2, minLines: 1, decoration: InputDecoration(border: InputBorder.none, labelText: 'Ek Not (isteğe bağlı)', hintText: 'Örn: Dikkatli taşınsın, kapıya bırakın, arayın...', labelStyle: TextStyle(fontSize: 11 * s, color: const Color(0xFF64748B)), hintStyle: TextStyle(fontSize: 9.5 * s, color: const Color(0xFFA0A9B9)))))]),
                ),
              ])),
              SizedBox(height: 16 * s),
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  style: ElevatedButton.styleFrom(backgroundColor: blue, disabledBackgroundColor: const Color(0xFF8FC6F7), foregroundColor: Colors.white, disabledForegroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20 * s)), elevation: 0),
                ),
              ),
            ]),
          );
  ...
  Widget _priceCard(double s) => Container(
        ...
        decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF168CF5), Color(0xFF3AA7FF)]), borderRadius: BorderRadius.circular(22 * s), boxShadow: const [BoxShadow(color: Color(0x25168CF5), blurRadius: 18, offset: Offset(0, 7))]),
        child: Row(children: [Container(width: 46 * s, height: 46 * s, decoration: BoxDecoration(color: Colors.white.withValues(alpha: .18), shape: BoxShape.circle), child: Icon(Icons.payments_rounded, color: Colors.white, size: 24 * s)), SizedBox(width: 12 * s), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Tahmini Ücret', style: TextStyle(fontSize: 12 * s, color: Colors.white.withValues(alpha: .86), fontWeight: FontWeight.w700)), Text('₺$estimatedPrice', style: TextStyle(fontSize: 27 * s, color: Colors.white, fontWeight: FontWeight.w900)), Text('${routeDistanceKm?.toStringAsFixed(1) ?? '—'} km • yaklaşık ${routeDurationMin ?? '—'} dk', style: TextStyle(fontSize: 10.5 * s, color: Colors.white.withValues(alpha: .82), fontWeight: FontWeight.w600))])), Container(padding: EdgeInsets.symmetric(horizontal: 10 * s, vertical: 7 * s), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14 * s)), child: Text(vehicle == 0 ? 'Motosiklet' : 'Araç', style: TextStyle(fontSize: 10 * s, color: blue, fontWeight: FontWeight.w900)))]),
      );

  Widget _addressField(double s, String title, String hint, AddressSelection? value, IconData icon, VoidCallback onTap) => InkWell(onTap: onTap, borderRadius: BorderRadius.circular(15 * s), child: Container(constraints: BoxConstraints(minHeight: 66 * s), padding: EdgeInsets.symmetric(horizontal: 11 * s, vertical: 8 * s), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(15 * s), border: Border.all(color: value != null ? const Color(0xFFB9DAFF) : const Color(0xFFE7ECF3))), child: Row(children: [Container(width: 32 * s, height: 32 * s, decoration: BoxDecoration(color: value != null ? const Color(0xFFEAF4FF) : const Color(0xFFF2F6FA), shape: BoxShape.circle), child: Icon(icon, color: value != null ? blue : const Color(0xFF64748B), size: 20 * s)), SizedBox(width: 10 * s), Expanded(child: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: TextStyle(fontSize: 10.5 * s, color: muted, fontWeight: FontWeight.w700)), SizedBox(height: 2 * s), Text(value?.displayName ?? hint, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 11.5 * s, color: value != null ? navy : const Color(0xFFA0A9B9), fontWeight: value != null ? FontWeight.w700 : FontWeight.w500))])), Icon(value != null ? Icons.edit_location_alt_rounded : Icons.chevron_right_rounded, color: value != null ? blue : const Color(0xFF69778B), size: 21 * s)])));
  ...
  Widget _vehicle(double s, int idx, String t, String sub, String time, String asset) {
    final sel = vehicle == idx;
  ...
  Widget _chip(double s, int idx, IconData i, String label) {
    final sel = detail == idx;
    return GestureDetector(onTap: () => setState(() => detail = idx), child: Container(height: 48 * s, decoration: BoxDecoration(color: sel ? const Color(0xFFF3F9FF) : Colors.white, borderRadius: BorderRadius.circular(14 * s), border: Border.all(color: sel ? blue : const Color(0xFFE5EAF2))), child: Center(child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(i, color: sel ? blue : const Color(0xFF607089), size: 18 * s), SizedBox(width: 5 * s), Text(label, style: TextStyle(fontSize: 10 * s, color: sel ? blue : const Color(0xFF607089), fontWeight: FontWeight.w700))]))));
  }

  Widget _select(double s, IconData i, String title, String value, VoidCallback onTap) => InkWell(
        onTap: onTap,
        borderRadius: BorderRadius.circular(15 * s),
        child: Container(height: 58 * s, padding: EdgeInsets.symmetric(horizontal: 10 * s), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(15 * s), border: Border.all(color: value == 'Seçiniz' ? const Color(0xFFE7ECF3) : const Color(0xFFB9DAFF))), child: Row(children: [Icon(i, color: value == 'Seçiniz' ? const Color(0xFF65748A) : blue, size: 18 * s), SizedBox(width: 8 * s), Expanded(child: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: TextStyle(fontSize: 9.4 * s, color: const Color(0xFF64748B))), Text(value, style: TextStyle(fontSize: 10.5 * s, color: value == 'Seçiniz' ? const Color(0xFF9AA4B5) : navy, fontWeight: value == 'Seçiniz' ? FontWeight.w500 : FontWeight.w800))])), Icon(Icons.keyboard_arrow_down_rounded, color: blue, size: 20 * s)])),
      );
}
